...
public class Homework1 {
    ...
    public void streamPipeline3() {
        countries.stream()
                .filter(country ->
                        country.getName().toLowerCase().charAt(0)
                                ==
    ...
    public void streamPipeline8() {
        countries.stream()
                .sorted(Comparator.comparingInt(countries -> countries.getTimezones().size()))
                .forEach(country -> System.out.printf("%s : %d", country.getName(), country.getTimezones().size()));
    }
...
ed by the language code "es").
    ...
    public Optional<Country> streamPipeline14() {
        return countries.stream()
                .filter(country -> country.getArea() != null)
                .max(Comparator.comparingDouble(country -> country.getArea().doubleValue()));
    }
